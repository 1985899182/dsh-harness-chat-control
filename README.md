...
当前里程碑：**`v0.2.63`**，适配上表中的 DSH Desktop `0.7.2` / Harness `0.1.2-alpha.1`，并内置 `dsh-better-sidebar@0.17.1` 的侧边对话实现；本版本修复了 Cordis 未注入属性导致的插件启动失败，以及隔离加载路径下消息 ID 缺失造成的 `undefined is already pending` 请求失败。升级 DSH 或 Harness 主版本后，请重新验证原生 Slots、侧边对话 API 和状态接口。
