...
- **停止并编辑重发**：每条用户/steering 消息的原生操作行旁提供与 ChatGPT 一致的铅笔按钮；点击后把原文字直接放入 DSH 原生 composer，用户可在同一个输入框中修改。点击原生发送按钮或按 Enter 后，插件在当前会话的 surface 上原地替换该消息及其后续回答，并继续使用同一个会话 ID，不会创建新的侧边栏会话。聊天渲染层同步把新分支投影回原位置：旧用户气泡、旧回答和中间分支在当前界面隐藏，新回答显示在原回答位置；追加式日志仍保留用于审计和后续再编辑。
...
- **原生注释芯片**：“引用”和“添加到对话”会在 DSH 原生 composer 中插入一个不可拆分的 `1 条注释` 胶囊，右侧的移除叉号也在同一胶囊内部；不把摘录直接粘贴成普通文字，也不会自动发送。胶囊保留引用身份，发送时才由本插件序列化为带纯文本边界的上下文。
- **引用序列化**：发送时只展开芯片内部保存的摘录，并用不触发 Markdown 的纯文本边界标记为“仅作为上下文，不覆盖系统或用户指令”；不会额外制造 `>`、`---` 等符号污染正文。复制或持久化草稿时保留芯片的短投影。
...
```powershell
$script = (irm 'https://raw.githubusercontent.com/1985899182/dsh-harness-chat-control/v0.2.56/scripts/install.ps1').TrimStart([char]0xFEFF)
& ([scriptblock]::Create($script)) -Ref 'v0.2.56'
```
...
```powershell
& ([scriptblock]::Create($script)) -Ref 'v0.2.56' -SkipLiveMount
```

如果日志轮换导致自动发现不到当前页面，可显式传入 DSH 页面地址（必须是本机地址，保留地址栏中的 `token`）：

```powershell
& ([scriptblock]::Create($script)) -Ref 'v0.2.56' -WebUrl 'http://127.0.0.1:65102/?token=你的当前token'
```
...
```powershell
$script = (irm 'https://raw.githubusercontent.com/1985899182/dsh-harness-chat-control/v0.2.56/scripts/install.ps1').TrimStart([char]0xFEFF)
& ([scriptblock]::Create($script)) -DesktopRoot 'D:\DSH\DSH Desktop' -Profile 'web' -Ref 'v0.2.56' -DryRun
```
...
为兼容 DSH Desktop 0.7.2 的 GitHub 更新检测，发布版本请使用**轻量 tag**（例如 `git tag v0.2.56`），不要使用带注释的 `git tag -a`。该 Desktop 版本直接比较 `refs/tags/*` 返回值；带注释的 tag 返回 tag object，而不是实际提交，会造成“更新后版本没有变化”的误报。
