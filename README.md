...
- **侧边模型选择**：侧边 composer 底部只显示一个当前 DSH 模型目录中的 provider/model 下拉选择（会隐藏 Better Sidebar 重复的原生模型徽标）。选择会在该 sidechat child session 上记录为下一次请求的模型（冷恢复线程也会在恢复瞬间应用），不会改变主会话的模型。
...
```powershell
$script = (irm 'https://raw.githubusercontent.com/1985899182/dsh-harness-chat-control/v0.2.27/scripts/install.ps1').TrimStart([char]0xFEFF)
& ([scriptblock]::Create($script)) -Ref 'v0.2.27'
```
...
```powershell
& ([scriptblock]::Create($script)) -Ref 'v0.2.27' -SkipLiveMount
```

如果日志轮换导致自动发现不到当前页面，可显式传入 DSH 页面地址（必须是本机地址，保留地址栏中的 `token`）：

```powershell
& ([scriptblock]::Create($script)) -Ref 'v0.2.27' -WebUrl 'http://127.0.0.1:65102/?token=你的当前token'
```
...
```powershell
$script = (irm 'https://raw.githubusercontent.com/1985899182/dsh-harness-chat-control/v0.2.27/scripts/install.ps1').TrimStart([char]0xFEFF)
& ([scriptblock]::Create($script)) -DesktopRoot 'D:\DSH\DSH Desktop' -Profile 'web' -Ref 'v0.2.27' -DryRun
```
...
为兼容 DSH Desktop 0.7.2 的 GitHub 更新检测，发布版本请使用**轻量 tag**（例如 `git tag v0.2.27`），不要使用带注释的 `git tag -a`。该 Desktop 版本直接比较 `refs/tags/*` 返回值；带注释的 tag 返回 tag object，而不是实际提交，会造成“更新后版本没有变化”的误报。
...
原生引用芯片依赖同一版本的 `@deepseek-ai/dsh-client-ui-input-trigger`，并通过 `conversation.input` 的 `slash/input-insert-reference` 事件接入 DSH composer。侧边对话依赖已安装并启用的 `dsh-better-sidebar@0.17.1`，使用其 `targetedOpen`、`stateSubscription` 和 `sidechat.*` API；该版本没有公开侧边 composer 草稿接口，因此插件只在侧边原生 composer 外层加一枚兼容胶囊和模型下拉，并在用户发送时接管引用序列化。模型下拉读取 `remote.session.modelCatalog()`，通过插件自己的同源受信路由调用 `llm.resolveCallConfig` 与 Session Agent 的下一请求选择接口。未启用 Better Sidebar 时，引用芯片仍可使用，但侧边对话入口会提示缺少该插件。
