...
- **原生侧边对话**：“侧栏问”“更多详情”和“在侧边聊天中提问”会打开 `dsh-better-sidebar@0.17.1` 自己的 sidechat 标签页。引用内容作为该独立线程的首条上下文提问发送，侧边线程继承当前会话上下文，但不会写入主会话。
...
引用输出时，插件会在芯片的发送序列化结果和 sidechat 首条消息中明确把引用内容标为“上下文”，避免其中的文本意外覆盖你的追问。侧边对话使用 Better Sidebar 的独立 child session，不会再出现插件自绘的第二个追问面板。
...
```powershell
$script = (irm 'https://raw.githubusercontent.com/1985899182/dsh-harness-chat-control/v0.2.16/scripts/install.ps1').TrimStart([char]0xFEFF)
& ([scriptblock]::Create($script)) -Ref 'v0.2.16'
```
...
```powershell
$script = (irm 'https://raw.githubusercontent.com/1985899182/dsh-harness-chat-control/v0.2.16/scripts/install.ps1').TrimStart([char]0xFEFF)
& ([scriptblock]::Create($script)) -DesktopRoot 'D:\DSH\DSH Desktop' -Profile 'web' -Ref 'v0.2.16' -DryRun
```
...
为兼容 DSH Desktop 0.7.2 的 GitHub 更新检测，发布版本请使用**轻量 tag**（例如 `git tag v0.2.16`），不要使用带注释的 `git tag -a`。该 Desktop 版本直接比较 `refs/tags/*` 返回值；带注释的 tag 返回 tag object，而不是实际提交，会造成“更新后版本没有变化”的误报。
...
原生引用芯片依赖同一版本的 `@deepseek-ai/dsh-client-ui-input-trigger`，并通过 `conversation.input` 的 `slash/input-insert-reference` 事件接入 DSH composer。侧边对话依赖已安装并启用的 `dsh-better-sidebar@0.17.1`，使用其 `targetedOpen`、`stateSubscription` 和 `sidechat.*` API；未启用 Better Sidebar 时，引用芯片仍可使用，但侧边对话入口会提示缺少该插件。
