...
const { createUserMessage: dshCreateUserMessage } = (() => {
...
const { admitEncodedImages, AttachmentError } = (() => {
  try { return peerRequire('@deepseek-ai/dsh-attachment') }
  catch { return { admitEncodedImages: undefined, AttachmentError: undefined } }
})()

export const name = 'dsh-harness-chat-control'

/**
 * Construct one message that is safe for every supported DSH loader.
 *
 * `@deepseek-ai/dsh-llm` normally supplies a fresh UUID, but a generation can
 * be evaluated by the desktop's isolated module loader before peer packages are
 * resolvable.  The old structural fallback then returned an id-less object;
 * Inbox.validate() treats two such messages as the same pending message and
 * reports `message "undefined" is already pending`.  Keep the fallback useful
 * for tests and cold loaders while guaranteeing the identity contract required
 * by Agent.followup()/inject().
 */
function createIdentifiedUserMessage(input) {
  let message
  try {
    if (typeof dshCreateUserMessage === 'function') message = dshCreateUserMessage(input)
  } catch {
    message = undefined
  }
  ...
  return Object.freeze({
    ...(message !== undefined && message !== null && typeof message === 'object' ? message : {}),
    ...input,
    role: 'user',
    id
  })
...
// The native DSH composer sends image bytes as canonical base64 before the
// host turns them into durable attachment references.  Model-selection and
// history requests stay small; sidechat prompts need the attachment budget.
const MAX_SIDECHAT_BODY_BYTES = 32 * 1024 * 1024
...
async function readJsonBody(req, maxBytes = MAX_MODEL_BODY_BYTES) {
  const chunks = []
  let total = 0
  for await (const chunk of req) {
    const buffer = Buffer.from(chunk)
    total += buffer.length
    if (total > maxBytes) throw new SidechatModelError('bad-request', 'request body too large', 413)
    chunks.push(buffer)
  }
...
const SIDECHAT_IMAGE_TYPES = new Set(['image/png', 'image/jpeg', 'image/webp', 'image/gif'])

/**
 * Keep the sidechat wire format identical to DSH's PromptContentPart image
 * shape.  The browser sends { mediaType, data, name }; the host admission
 * service then owns validation, limits and durable attachment persistence.
 */
function sidechatImageParts(images) {
  if (images === undefined) return []
  if (!Array.isArray(images)) throw new SidechatModelError('bad-request', '"images" must be an array')
  if (images.length > 32) throw new SidechatModelError('bad-request', 'too many images')
  return images.map((image) => {
    if (image === null || typeof image !== 'object') {
      throw new SidechatModelError('bad-request', 'invalid image payload')
    }
    const mediaType = image.mediaType
    const data = image.data
    if (!SIDECHAT_IMAGE_TYPES.has(mediaType) || typeof data !== 'string' || data === '') {
      throw new SidechatModelError('bad-request', 'invalid image payload')
    }
    const name = image.name
    if (name !== undefined && (typeof name !== 'string' || name.length > 256)) {
      throw new SidechatModelError('bad-request', 'invalid image name')
    }
    return {
      type: 'image',
      mediaType,
...
/** Admit browser image parts through DSH's own attachment store. */
async function durableSidechatImageParts(ctx, images) {
  if (images.length === 0) return []
  const attachments = getService(ctx, 'attachments')
  if (attachments === undefined || typeof attachments.saveImages !== 'function') {
    throw new SidechatModelError('unavailable', '当前 DSH 没有可用的图片附件服务', 503)
  }
  try {
    // Production DSH always supplies the shared admission helper.  The
    // structural fallback keeps `npm test` useful without bundling DSH's
    // private peer package into this plugin.
    const refs = typeof admitEncodedImages === 'function'
      ? await admitEncodedImages(attachments, images)
      : await attachments.saveImages(images.map((image) => {
        const decoded = Buffer.from(image.data, 'base64')
        if (decoded.length === 0 || decoded.toString('base64') !== image.data) {
          throw new SidechatModelError('attachment-error', 'Image upload is not canonical base64.', 400)
        }
        return {
    ...
    if (AttachmentError !== undefined && error instanceof AttachmentError) {
      throw new SidechatModelError('attachment-error', error.message, 400)
    }
    throw new SidechatModelError('attachment-error', error instanceof Error ? error.message : String(error), 400)
  }
}

/** Match the native SessionController image-capability gate for a child Agent. */
async function assertSidechatImageModel(ctx, agent, hasImages) {
  if (!hasImages) return
  const llm = getService(ctx, 'llm')
  const controller = getService(ctx, 'sessionController')
  const selection = controller?.agents?.selectionFor?.(agent)?.current
  if (typeof llm?.resolveModelInfo !== 'function' || selection === undefined) return
  try {
    ...
    if (Array.isArray(info?.inputModalities) && !info.inputModalities.includes('image')) {
      throw new SidechatModelError('attachment-error', `模型 "${selection.model}" 不支持图片输入`, 400)
    }
  } catch (error) {
    if (error instanceof SidechatModelError) throw error
    // Keep the same fail-closed behavior as the native SessionController when
    // a model capability lookup fails during an image admission.
    throw new SidechatModelError('attachment-error', error instanceof Error ? error.message : String(error), 400)
  }
}

/** Text plus durable image blocks, in the exact order used by DSH InputBar. */
async function sidechatPromptContent(ctx, text, images) {
  return [...await durableSidechatImageParts(ctx, images), ...text === '' ? [] : textPrompt(text)]
}
/** Admit one user message to a live agent through the stock followup path. */
function admitFollowup(agent, blocks) {
	const message = createIdentifiedUserMessage({
		content: blocks,
		source: { kind: "user" }
	});
...
*/
function admitFirstContact(agent, injectionText, question, blocks = textPrompt(question)) {
	agent.inject(createIdentifiedUserMessage({
...
function buildSidechatApi(ctx) {
	return {
		...
		"sidechat.prompt": async (payload) => {
			...
			const images = sidechatImageParts(payload.images);
			const rawText = payload?.text;
			if (typeof rawText !== "string") throw new SidechatModelError("bad-request", "missing or invalid \"text\"");
			const text = rawText.trim();
			if (text === "" && images.length === 0) throw new SidechatModelError("bad-request", "text or images are required");
			...
			const admit = async () => {
				await assertSidechatImageModel(ctx, agent, images.length > 0);
				const blocks = await sidechatPromptContent(ctx, text, images);
				if (boundaryDelivered(agent.session.events)) admitFollowup(agent, blocks);
				else {
			...
			// The standard DSH session prompt checks model image capability before
			// admission and serializes uploads under the Agent's image-admission
			// mutex.  Sidechat owns a private child, so mirror that same host seam
			// around the Better Sidebar follow-up instead of dropping image blocks.
			const agents = getService(ctx, "agents");
			const controllerAgents = getService(agent?.ctx, 'sessionController')?.agents
				|| getService(ctx, 'sessionController')?.agents
				|| agents;
			const admitted = typeof controllerAgents?.serializeImageAdmission === "function"
				? await controllerAgents.serializeImageAdmission(agent, admit)
				: await admit();
			return admitted;
		},
...
function createSidechatModelRoute(ctx) {
  ...
  function installSelection(agent, selection) {
    // A child Agent owns a scoped Cordis context.  Newer DSH generations
    // expose SessionController through that scope; older ones expose the same
    // AgentController only on the root.  Resolve both so the native
    // selectForNextRequest seam is never silently skipped.
    const controller = getService(agent?.ctx, 'sessionController') || getService(ctx, 'sessionController')
    const agents = controller?.agents || getService(ctx, 'agents')
...
export function apply(ctx) {
  const registerRoute = () => {
    ...
    const sidechatApiHandler = (method, handler) => async (req, res) => {
      if (!trustedRequest(ctx, req)) {
        writeJson(res, 403, { ok: false, error: { code: 'forbidden', message: 'forbidden' } })
        return
      }
      ...
      try {
        const payload = await readJsonBody(req, method === 'sidechat.prompt'
          ? MAX_SIDECHAT_BODY_BYTES
    ...
    try {
      ...
      for (const [method, handler] of Object.entries(sidechatApi)) {
        disposeSidechatApi.push(webServer.register({
          ...
          handler: sidechatApiHandler(method, handler)
        }))
      }
    } catch (error) {
