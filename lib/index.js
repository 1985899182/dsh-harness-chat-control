...
function createSidechatHistoryRoute(ctx) {
  ...
  const handler = async (req, res) => {
    ...
    if (req.method === 'GET' && new URL(req.url ?? '/', 'http://x').searchParams.get('debug') === '1') {
      const clientModules = ctx?.clientModules || getService(ctx, 'clientModules')
      const graph = typeof clientModules?.graph === 'function' ? clientModules.graph() : undefined
      const row = Array.isArray(graph?.entries) ? graph.entries.find((entry) => entry.id === name) : undefined
      const table = typeof clientModules?.table?.get === 'function' ? clientModules.table.get(name) : undefined
      const sources = typeof clientModules?.sources?.values === 'function'
        ? [...clientModules.sources.values()]
            .filter((source) => source.packageName === name)
            .map((source) => ({ loaderName: source.loaderName, baseUrl: source.baseUrl, sourceKey: source.sourceKey, clientPath: source.meta?.clientPath }))
        : []
      const packageMeta = typeof clientModules?.pkgMeta?.entries === 'function'
        ? [...clientModules.pkgMeta.entries()]
            .filter(([key, value]) => key.endsWith(`\u0000${name}`) || value?.packageName === name)
            .map(([key, value]) => ({ key, packageName: value?.packageName, clientPath: value?.meta?.clientPath }))
        : []
      writeOk(res, {
        service: clientModules?.constructor?.name,
        clientPath: typeof clientModules?.clientPath === 'function' ? clientModules.clientPath(name) : undefined,
        row,
        table: table === undefined ? undefined : { baseline: table.baseline, entry: table.entry, meta: table.meta },
        sources,
        packageMeta,
      })
      return
