 ...
 * sidechat child: model selection, the current permission preset, and a
 * history page for the custom sidechat children.  The session-edit route is
 * separate: it arms the already-open ordinary Session for one surface
 * replacement so the browser can use the native prompt admission path. Every
 * route validates its target before reading or changing anything; none of them
 * reads credentials.
 */
...
function sessionEditRange(session, startSeq, targetText) {
  ...
  const surfaceNodes = Array.isArray(session?.surface?.nodes)
    ? [...session.surface.nodes]
    : []
  if (surfaceNodes.length === 0) return undefined

  let targetSeq = Number.isSafeInteger(startSeq) && startSeq >= 0 ? startSeq : undefined
  const isUserMessage = (seq) => {
    const event = events.find((candidate) => candidate?.seq === seq) || events[seq]
  ...
  if (targetSeq === undefined || !surfaceNodes.includes(targetSeq) || !isUserMessage(targetSeq)) {
    ...
    for (let index = surfaceNodes.length - 1; index >= 0; index -= 1) {
      ...
      if (targetText === '' || textFromMessage(events[seq]?.data) === targetText) {
        targetSeq = seq
        break
      }
    }
...
function createSessionEditRoute(ctx) {
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      const agent = getService(ctx, 'agents')?.get?.(sessionId)
