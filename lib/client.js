...
`
    ...
    function NativeSidechatComposer({ ctx, scope, tab, draftStore, onRefresh }) {
      ...
      const modelInjected = React.useMemo(() => {
        if (childId === undefined || typeof modelEntry?.inject !== 'function') return undefined
        try { return modelEntry.inject(childId) } catch { return undefined }
      }, [modelEntry, childId])
      ...
      // the injected loader when the child session becomes available.  Without
      // this call the native selector stays stuck on "正在加载模型…" and the
      // side conversation cannot resolve a model for its first request.
      React.useEffect(() => {
      ...
      const renderSlot = React.useCallback((key, owner = {}) => {
        if (key === 'conversation.input.model' && typeof modelEntry?.component === 'function' && modelInjected !== undefined) {
          return h(modelEntry.component, { ...modelInjected, ...owner, t: modelT })
        }
        ...
        return null
      }, [attachmentEntry, modelEntry, modelInjected, modelT, planEntry, planInjected, planT, t, useProjection])
    ...
    function apply(ctx) {
      ...
      const disposeSidechatComposer = installSidechatComposer(ctx, sideChatDrafts)
      ...
      ctx.effect?.(() => disposeSidechatComposer, 'dsh-harness-chat-control: native sidechat composer')
