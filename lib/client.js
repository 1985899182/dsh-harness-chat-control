...
.dshhc-sidechat-draft-error {
  flex: none; margin: 2px 0 3px; color: var(--dsw-alias-state-error-primary, #d32f2f);
...
`
     ...
     */
    function createSideChatDraftController() {
      ...
      function composerFor(field) {
        return closestElement(field, (element) => classNameOf(element).includes('sidechatComposer'))
          || field?.parentElement
      }
      ...
      function showDraftError(draft, cause) {
        ...
        composer.insertBefore(error, bar || null)
      }
      ...
      function bindField(draft, field, composer) {
        ...
        if (draft.referenceText !== '') {
          ...
          if (row === null || row === undefined) {
            ...
            composer.insertBefore(row, composer.firstChild || null)
          }
          const chip = row.querySelector?.('.dshhc-sidechat-reference-chip')
          if (chip !== null && chip !== undefined) {
            chip.title = draft.referenceText
            chip.querySelector?.('.dshhc-sidechat-reference-label')
              && (chip.querySelector('.dshhc-sidechat-reference-label').textContent = referencePreview(draft.referenceText))
          }
        }
      ...
      function handleDocumentClick(event) {
        ...
        if (button === undefined || button === null) return
        const composer = closestElement(button, (element) => classNameOf(element).includes('sidechatComposer'))
    ...
    function apply(ctx) {
      ...
      const sideChatDrafts = createSideChatDraftController()
