...
.dshhc-sidechat-model-select {
  position: absolute; z-index: 2; right: 44px; bottom: 6px; box-sizing: border-box;
  min-width: 0; max-width: calc(100% - 88px); height: 28px; margin: 0;
  padding: 0 22px 0 8px; color: var(--dsw-alias-label-secondary, #666);
  background: var(--dsw-alias-interactive-bg-hover, rgba(0,0,0,.06));
  border: 1px solid transparent; border-radius: 14px; cursor: pointer; font: inherit;
  font-size: 12px; line-height: 26px; text-overflow: ellipsis; white-space: nowrap;
}
[class*="sidechatComposer"][data-dsh-harness-model] { position: relative; }
...
[class*="sidechatComposer"][data-dsh-harness-model] [class*="sidechatComposerMeta"] { visibility: hidden !important; }
.dshhc-sidechat-model-select[data-dsh-harness-model-loading] { cursor: wait; }
.dshhc-sidechat-model-select:hover:not(:disabled) { color: var(--dsw-alias-label-primary, #202123); background: var(--dsw-alias-interactive-bg-active, rgba(0,0,0,.1)); }
...
`
    ...
    function createSideChatModelController(ctx) {
      ...
      function setLoading(binding, message = '正在加载模型…') {
        ...
        if (binding.renderedSignature !== `loading:${message}`) {
          clearOptions(select)
          const option = doc.createElement('option')
          option.value = ''
          option.textContent = message
          select.appendChild(option)
          binding.renderedSignature = `loading:${message}`
        }
      ...
      function populate(binding) {
        ...
        const signature = JSON.stringify(options.map((entry) => [entry.label, entry.value]))
        if (options.length > 0 && binding.renderedSignature !== signature) {
          clearOptions(select)
          for (const entry of options) addOption(select, entry)
          binding.renderedSignature = signature
        }
        if (options.length === 0) {
          if (binding.renderedSignature !== 'empty') {
            clearOptions(select)
            const option = doc.createElement('option')
            option.value = ''
            option.textContent = '暂无可用模型'
            select.appendChild(option)
            binding.renderedSignature = 'empty'
          }
          select.disabled = true
      ...
      async function selectModel(binding) {
        ...
        const previous = selections.get(binding.childId) || defaultSelection(catalog)
        binding.busy = true
        binding.pendingSelection = next
        binding.select.disabled = true
        binding.select.setAttribute?.('aria-busy', 'true')
        try {
          const result = await callSideChatModel({ childId: binding.childId, ...next })
          const selected = result?.selected
          const normalized = typeof selected?.provider === 'string' && typeof selected?.model === 'string'
            ? {
            ...
            : next
          selections.set(binding.childId, normalized)
          binding.pendingSelection = normalized
          binding.error = ''
        } catch (error) {
      ...
      function ensureBinding(composer) {
        ...
        if (binding === undefined) {
          ...
          select.className = 'dshhc-sidechat-model-select'
          ...
          // The outer composer is React-managed only through its native
          // textarea and bar children.  Keep this control as a sibling so a
          // React render never tries to reconcile or delete it.
        ...
        populate(binding)
        if (binding.childId !== undefined) void hydrateInfo(binding, binding.childId)
        return binding
      }
      ...
      function scan() {
        ...
        for (const [composer, binding] of bindings) {
          ...
          try { binding.select.remove?.() } catch {}
