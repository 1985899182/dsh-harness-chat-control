...
`
    ...
    function selectionQuoteDraft(text) {
      return `引用的对话内容：\n${quoteLines(text)}\n\n请基于以上内容继续回答：`
    }
    ...
    function createSidePanelStore() {
      let snapshot = Object.freeze({ open: false, reference: null, initialQuestion: '', revision: 0 })
      const listeners = new Set()
      ...
      return {
        ...
        open(reference, initialQuestion = '') {
          publish({ open: true, reference, initialQuestion: clampText(initialQuestion, 1000) })
        },
        close() {
          publish({ open: false, reference: snapshot.reference, initialQuestion: '' })
        },
        clearReference() {
          publish({
            open: true,
    ...
    function SelectionToolbar({ selectionStore, inputBridge, panelStore, useSessions }) {
      ...
      function addToConversation() {
        ...
        if (record?.actions?.setDraft) {
          const existing = typeof record.draft === 'string' ? record.draft.trimEnd() : ''
          const quote = selectionQuoteDraft(panel.text)
          const next = existing === '' ? quote : `${existing}\n\n${quote}`
          record.actions.setDraft(next)
        } else if (sessionId !== null && sessionId !== undefined) {
          // The composer may be temporarily unmounted during a layout switch;
          // preserve the intended action by opening the contextual side panel.
          panelStore.open(
            { sessionId, text: panel.text },
            '请把这段选中的内容加入当前对话上下文，然后继续回答。'
          )
        }
        finishAction()
      }
      ...
      function askInSideChat() {
        if (sessionId !== null && sessionId !== undefined) panelStore.open({ sessionId, text: panel.text }, '')
        finishAction()
      }
    ...
    function AnswerActions({ messageId, sessionId, useChat, useInput, inputActions, inputBridge, openSide }) {
    ...
    function SideQuestionPanel({ panelStore, submitQuestion }) {
      ...
      React.useEffect(() => {
        setQuestion(panel.initialQuestion || '')
        setError(null)
        setStatus(null)
      }, [panel.revision])
    ...
    function ShellOverlay({ panelStore, submitQuestion, selectionStore, inputBridge, useSessions }) {
      return h(React.Fragment, null,
        h(SideQuestionPanel, { panelStore, submitQuestion }),
        h(SelectionToolbar, { selectionStore, inputBridge, panelStore, useSessions })
      )
    }
    ...
    function apply(ctx) {
      ...
      function openPanel(sessionId, text, initialQuestion = '') {
        panelStore.open({ sessionId, text: clampText(text) }, initialQuestion)
      }
      ...
      slots.inject('shell.overlay', () => slots.register({
        ...
        order: 80,
        inject: () => ({ panelStore, submitQuestion: submitSideQuestion, selectionStore, inputBridge })
      }, ShellOverlay))
    }
