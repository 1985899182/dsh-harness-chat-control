    ...
    function installRewriteRenderers(slots, sessions) {
      ...
      return slots.inject('conversation.chat.node', () => {
        ...
        for (const entry of slots.entries('conversation.chat.node')) {
          ...
          if (typeof key !== 'string' || nativeByKey.has(key) || !isReactComponent(entry.component)) continue
          // The plugin's own entries are installed at a negative priority;
          // capture the native priority-0 renderer as the delegation target.
          if ((entry.options.priority ?? 0) === 0) nativeByKey.set(key, entry)
        }
        const disposers = []
        const mutated = []
        for (const key of REWRITE_CHAT_NODE_KEYS) {
          const nativeEntry = nativeByKey.get(key)
          if (nativeEntry === undefined) continue
          const nativeComponent = nativeEntry.component
          const rewriteComponent = createRewriteNodeView(nativeComponent, sessions)
          // `command` and `turn-tail` own child Slots.  Re-registering those
          // keys would collide with the existing child declarations and, more
          // importantly, would remove the native renderSlot authorization.
          // Wrap the existing entry in place so its children, store, locale,
          // and scoped inject contract remain exactly the native ones.
          if (nativeEntry.children !== undefined) {
            nativeEntry.component = rewriteComponent
            mutated.push({ entry: nativeEntry, component: nativeComponent, rewriteComponent })
            continue
          }
        ...
        // A keyed-slot mutation needs one registry pulse so already-mounted
        // ChatNodeSeat outlets re-read the component field immediately.
        let pulseDispose
        if (mutated.length > 0) {
          pulseDispose = slots.register({
            name: 'conversation.chat.node',
            key: '__dshhc-rewrite-pulse__',
            priority: REWRITE_CHAT_NODE_PRIORITY
          }, () => null)
        }
        return () => {
          ...
          for (const { entry, component, rewriteComponent } of mutated.reverse()) {
            if (entry.component === rewriteComponent) entry.component = component
          }
        }
      })
    }
