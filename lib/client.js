...
`
    ...
    async function callSideChatPermission(payload) {
    ...
    function createSideChatModelController(ctx) {
      ...
      function permissionName(entry) {
        ...
        return String(entry?.value || 'Workspace Write')
      }
      ...
      async function selectPermission(binding, value) {
        ...
        try {
          const result = await callSideChatPermission({ childId: binding.childId, permission: value })
      ...
      async function hydratePermission(binding, childId) {
        ...
        binding.permissionPromise = callSideChatPermission({ childId }).then((result) => {
          if (binding.childId !== childId) return
